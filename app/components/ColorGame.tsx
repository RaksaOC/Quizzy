...
import { Palette, Star, CheckCircle, XCircle } from 'lucide-react'
...
export default function ColorGame({ onBack }: ColorGameProps) {
    ...
    return (
        <GameLayout
            ...
            gradientColors={GAME_COLORS.colors}
        >
            <div className="space-y-8">
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {/* Question */}
                        <div className="mb-8">
                            <div className="relative rounded-2xl overflow-hidden bg-fuchsia-600/90 backdrop-blur-sm aspect-video max-w-2xl mx-auto mb-6 border-2 border-white/20 shadow-lg">
                            ...
                            <div className="flex justify-center gap-4 mb-8">
                                {currentQuestion.pattern.map((emoji, index) => (
                                    <div
                                        key={index}
                                        className="w-16 h-16 flex items-center justify-center text-4xl bg-white/10 rounded-lg backdrop-blur-sm"
                                    >
                                        {emoji}
                                    </div>
                                ))}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-2xl mx-auto">
                            {currentQuestion.options.map((pattern: string[], index: number) => {
                                ...
                                return (
                                    <motion.button
                                        ...
                                        className={`
                                            relative p-6 rounded-xl text-center font-medium
                                            ${!selectedAnswer
                                                ? 'bg-fuchsia-500/80 text-white hover:bg-fuchsia-600/80 border-2 border-white/20'
                                                : isSelected
                                                    ? isCorrect
                                                    ...
                                                    : isCorrect && showExplanation
                                                        ? 'bg-green-500/90 text-white border-2 border-green-400/50'
                                                        : 'bg-fuchsia-400/50 text-white/60 border-2 border-white/10'
                                            }
                                            transition-all duration-300 shadow-lg
                                        `}
                                    >
                                        <div className="flex items-center justify-center gap-3">
                                            {pattern.map((emoji, emojiIndex) => (
                        ...
                        <AnimatePresence>
                            {showExplanation && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0, y: -20 }}
                                    className="mt-8 p-6 bg-fuchsia-600/80 backdrop-blur-sm rounded-xl text-white border-2 border-white/20 shadow-lg"
                                >
